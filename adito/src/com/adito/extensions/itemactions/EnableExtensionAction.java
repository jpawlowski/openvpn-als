...
/**
 */
public class EnableExtensionAction extends TableItemAction {

    /**
     * Default constructor
     */
    public EnableExtensionAction() {
        super("enable", "extensions", 300, "", false, SessionInfo.MANAGEMENT_CONSOLE_CONTEXT,
    ...
    public String getPath(AvailableTableItemAction availableItem) {
        ...
        return "/showExtensionStore.do?actionTarget=enable&id=" + item.getBundle().getId() + "&subForm=" + item.getSubFormName();
    }

    @Override
    public boolean isEnabled(AvailableTableItemAction availableItem) {
        ExtensionBundleItem item = (ExtensionBundleItem) availableItem.getRowItem();
        return item.getBundle().canEnable() && !item.getSubFormName().equals("updateableExtensionsForm");
    }
}