...
package com.ovpnals.reverseproxy;
...
import com.ovpnals.webforwards.AbstractAuthenticatingWebForwardHandler;
import com.ovpnals.webforwards.ReverseProxyWebForward;
...
public class ReverseProxyMethodHandler extends AbstractAuthenticatingWebForwardHandler implements RequestHandler {
	...
	final static String sessionCookie = SystemProperties.get("ovpnals.cookie", "JSESSIONID");
				...
				if (header.equalsIgnoreCase("cookie")) {
					...
					for (int i = 0; i < cookieVals.length; i++) {
						...
						if (cn.equals(Constants.LOGON_TICKET) || cn.equals(Constants.DOMAIN_LOGON_TICKET)
							|| cn.equals(SystemProperties.get("ovpnals.cookie", "SSLX_SSESHID"))) {
							if (log.isDebugEnabled())
