 ...
 * {@link net.openvpn.als.extensions.ApplicationLauncher} extensions that have a
 * type of {@link net.openvpn.als.applications.types.HtmlType}.
 * <p>
 ...
 * <p>
 * A second optional <b>openvpnals</b> parameter may be provided that should
...
public class GetHTMLApplicationAction extends AuthenticatedAction {
    ...
    public ActionForward onExecute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
        ...
        String url = request.getParameter("openvpnals");
        if (url != null) {
            ...
            if (host != null) {
                ...
                throw new Exception("No openvpnals parameter supplied.");
            }
        }
        final URL openvpnalsUrl = new URL(url);
        engine.addPattern("\\$\\{openvpnals:[^\\}]*\\}", new Replacer() {
            public String getReplacement(Pattern pattern, Matcher matcher, String sequence) {
                String match = matcher.group();
                try {
                    String param = match.substring(14, match.length() - 1);
                    if (param.equals("host")) {
                        return openvpnalsUrl.getHost();
                    } else if (param.equals("port")) {
                        return String.valueOf(openvpnalsUrl.getPort() == -1 ? (openvpnalsUrl.getProtocol().equals("https") ? 443
                                        : 80) : openvpnalsUrl.getPort());
                    } else if (param.equals("protocol")) {
                        return openvpnalsUrl.getProtocol();
                    } else {
