...
/**
 * Specialisation of {@link CoreException} for exceptions generate during use of
 * <i>Extensions</i>.
 */
public class ExtensionException extends CoreException {
	...
	/**
	 * Unknown extension type. Arg0 is the requested type, Arg1 is the id of the
	 * extension bundle attempting to use the type
	 */
	...
	/**
	 * The version of OpenVPN-ALS hosting the plugin is not sufficient. Arg0 is
	 * the plugin name, Arg1 is the required version
	 */
	public static final int INSUFFICIENT_OVPNALS_HOST_VERSION = 9;

	/**
	 * The plugin instance could not be created for some reason. Arg0 will be
	 * the plugin name, Arg1 will be the plugin class name and Arg2 will be the
	 * error text.
	 */
	public static final int FAILED_TO_CREATE_PLUGIN_INSTANCE = 10;

	/**
	 * The bundle is not in the required state for the requested operation. Arg0
	 * will contain the bundle name and Arg1 will contain any additional
	 * information.
	 */
	...
	/**
	 * An attempt was made to start an extension that requires a dependency that
	 * is not installed. Arg0 is the dependency name required, Arg1 is the
	 * bundle that requries the dependency.
	 */
	public static final int DEPENDENCY_NOT_INSTALLED = 13;

	/**
	 * An attempt was made to start an extension that requires a dependency that
	 * is not started. Arg0 is the dependency name required, Arg1 is the bundle
	 * that requries the dependency.
	 */
	...
	/**
	 * Attempt to load an extension that replaces a <i>devExtens</i>
	 */
	public static final int CANNOT_REPLACE_DEV_EXTENSION = 16;
	...
	protected MessageResources getMessageResources(String bundleName) {
		...
		if(mr == null) {
			...
			mr = fact.createResources("com.ovpnals.errors.ApplicationResources");
		}
		return mr;
	}
