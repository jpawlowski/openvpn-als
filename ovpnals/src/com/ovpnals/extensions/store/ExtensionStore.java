...
package com.ovpnals.extensions.store;
...
import com.ovpnals.boot.RepositoryFactory;
...
import com.ovpnals.core.LicenseAgreement;
...
import com.ovpnals.tasks.TaskInputStream;
import com.ovpnals.tasks.TaskProgressBar;
import com.ovpnals.util.ZipExtract;
...
public class ExtensionStore {
	 ...
	 */
	public static final String AGENT_EXTENSION_BUNDLE_ID = "ovpnals-agent";
	...
	public void init(File basedir) throws IOException {
		...
		repositoryBacked = "true".equals(SystemProperties.get("ovpnals.extensions.repositoryBacked", "true"));
	...
	void checkExtensionsForInstallation() {
		...
		PropertyList forceInstalls = new PropertyList(SystemProperties.get("ovpnals.forceInstallers", ""));
		for (ExtensionBundle bundle : extensionBundlesList) {
			if (bundle.getInstaller().getOpCount() > 0 && bundle.getStatus().isStartedOrActivated()) {
				...
				if (force || ver.equals("") || !ver.equals(bundle.getVersion().toString())) {
				    if(force) {
                        log.info("Will run installer for " + bundle.getId() + " because it has been forced by the ovpnals.foreceInstallers property.");
				    }
				    else if (ver.equals("")) {
	...
	private static URL getDownloadURL(String id, String version) {
		try {
            String location = SystemProperties.get("ovpnals.downloadableApplicationStore.location", ExtensionStore.HTTP_3SP_COM_APPSTORE);
            location += Util.urlEncode(id) + "/" + Util.urlEncode(version) + "/" + Util.urlEncode(id) + ".zip";
			return new URL(location);
		} catch (MalformedURLException murle) {
			try {
				String path = SystemProperties.get("ovpnals.downloadableApplications.location");
				path = path.replaceAll("\\$\\{id\\}", id);
                path = path.replaceAll("\\$\\{version\\}", version);
				return new File(path).toURL();
			} catch (MalformedURLException e) {
				log.error("Invalid downloadable extension location specified in system property ovpnals.downloadableApplicationStore.location, '" + SystemProperties.get("ovpnals.downloadableApplicationStore.location")
	...
	public static URL getStoreDownloadURL(String appStoreLocation, VersionInfo.Version version) {
		try {
			String location = SystemProperties.get("ovpnals.downloadableApplicationStore.location", appStoreLocation);
            location += "core/" + Util.urlEncode(version.toString()) + "/store.xml";
			return new URL(location);
		} catch (MalformedURLException murle) {
			try {
				return new File(SystemProperties.get("ovpnals.downloadableApplicationStore.location")).toURL();
			} catch (MalformedURLException e) {
				log.error("Invalid downloadable extension store location specified in system property ovpnals.downloadableApplicationStore.location, '" + SystemProperties.get("ovpnals.downloadableApplicationStore.location")
					+ "'. Must be either a URL or the file path of the store descriptor file.");
			}
		}
		return null;
	...
	private void loadAll() throws Exception {
		...
		String descriptors = SystemProperties.get("ovpnals.additionalDescriptors", "");
    ...
    public static VersionInfo.Version getWorkingVersion() {
        VersionInfo.Version version = new VersionInfo.Version(SystemProperties.get("ovpnals.forceVersion", ContextHolder.getContext().getVersion().toString()));
        return version;
    }
	...
	public ExtensionBundle installExtensionFromStore(final String id, InputStream in, HttpServletRequest request, long contentLength)
		...
		try {
			...
			if (bundle.getRequiredHostVersion() != null && bundle.getRequiredHostVersion().compareTo(context.getVersion()) > 0) {
				throw new ExtensionException(ExtensionException.INSUFFICIENT_OVPNALS_HOST_VERSION,
								bundle.getId(),
...
//		}
	...
	private void addAdditionalWebResource() {
		//
		String additionalWebResources = SystemProperties.get("ovpnals.additionalWebResourceDirectories", "");
	...
	private void addAdditionalClasspath() {
		...
		// Add any additional class path elements
		StringTokenizer t = new StringTokenizer(SystemProperties.get("ovpnals.additionalClasspath", ""), ",");
		while (t.hasMoreTokens()) {
			try {
	...
	private void loadDevExtensions() {
		...
		String extensionList = SystemProperties.get("ovpnals.devExtensions", "");
		...
		while (t.hasMoreTokens()) {
			...
			if (ext.equalsIgnoreCase("all")) {
				...
				File[] dirs = f.listFiles(new FileFilter() {
					public boolean accept(File pathname) {
						File f = new File(pathname, "extensions");
						return f.exists() && f.isDirectory() && pathname.getName().indexOf("ovpnals-enterprise-") != -1;
					}
				});
				for (int i = 0; dirs != null && i < dirs.length; i++) {
					devExtensions.add(dirs[i].getName());
				...
				File[] dirs = f.listFiles(new FileFilter() {
					public boolean accept(File pathname) {
						File f = new File(pathname, "extensions");
						return f.exists() && f.isDirectory() && pathname.getName().indexOf("ovpnals-community-") != -1;
					}
				});
				for (int i = 0; dirs != null && i < dirs.length; i++) {
					devExtensions.add(dirs[i].getName());
