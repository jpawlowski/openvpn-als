...
public class CoreRequestProcessor extends TilesRequestProcessor {
	...
	private void setDefaultLocale(HttpServletRequest request, HttpSession session) {
		...
		try {
            if (Util.isNullOrTrimmedBlank(SystemProperties.get("ovpnals.defaultLanguage", ""))) {
    		    Cookie[] c = request.getCookies();
    		    if (c != null) {
    		        for (int i = 0; i < c.length; i++) {
    		            if (c[i].getName().equals(SystemProperties.get("ovpnals.cookie", "SSLX_SSESHID") + "_LANG")) {
                ...
                defaultLocale = SystemProperties.get("ovpnals.defaultLanguage", "");
            }
		    Locale locale;
    ...
    private static boolean isRunningUnitTests() {
        String isRunningUnitTests = SystemProperties.get("ovpnals.testing", "false");
