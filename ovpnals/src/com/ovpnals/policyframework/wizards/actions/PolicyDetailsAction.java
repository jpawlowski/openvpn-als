...
package com.ovpnals.policyframework.wizards.actions;
...
/**
 * <p>
 * Action for policy resource details.
 */
public class PolicyDetailsAction extends AbstractWizardAction {

    /**
     * Constructor
     */
    public PolicyDetailsAction() {
        super(PolicyConstants.POLICY_RESOURCE_TYPE, new Permission[] { PolicyConstants.PERM_CREATE_EDIT_AND_ASSIGN });
    }
    ...
    protected AbstractWizardSequence createWizardSequence(ActionMapping mapping, ActionForm form, HttpServletRequest request,
        ...
        DefaultWizardSequence seq = new DefaultWizardSequence(fwd, "policyframework", "policyWizard", CoreUtil.getReferer(request),
                        "policyWizard", this.getSessionInfo(request));
        seq.addStep(new WizardStep("/policyDetails.do", true));
        seq.addStep(new WizardStep("/policyPrincipalSelection.do"));
        seq.addStep(new WizardStep("/policySummary.do"));
        return seq;
    }
}
