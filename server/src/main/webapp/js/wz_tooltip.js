...
var ttBgColor     = "#feffc3";
var ttBgImg       = "";           // path to background image;
var ttBorderColor = "#edeeb2";
var ttBorderWidth = 1;
...
var ttOpacity     = 93;          // opacity of tooltip in percent (must be integer between 0 and 100)
...
function tt_Show(t_e, t_id, contentLocation, t_sup, t_clk, t_delay, t_fix, t_left, t_offx, t_offy, t_static, t_sticky, t_temp)
{
	if(tt_obj) tt_Hide();
	...
	if(tt_obj)
	{
		...
		if(contentLocation != '') {
			var t_text = $(t_id + '_text');
			if(t_text) {
				t_text.replace(loadXMLDoc(unescape(contentLocation)));
			}
		}
		...
		{--j;
			if(typeof (t_tj = tags[j]).onmouseover == "function" && t_tj.onmouseover.toString().indexOf(esc) != -1 && !tt_n6 || tt_n6 && (over = t_tj.getAttribute("onmouseover")) && over.indexOf(esc) != -1)
			{
				...
				if(txt.length > 0 && txt.substring(0,1) == '!') {
					contentLocation = txt.substring(1);
					txt = '';
				}
				htm += tt_Htm(
					t_tj,
				...
				t_tj.onmouseover = new Function('e',
					...
					'"tOoLtIp' +i+''+j+ '","' + escape(contentLocation) + '",'+
