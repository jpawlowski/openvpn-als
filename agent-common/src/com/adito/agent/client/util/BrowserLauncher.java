 ...
 * <p>
 * <p>
 * Changes made by Brett Smith ( <a href="mailto:brett@localhost">brett@localhost.
 * </a> to allow a command to be specified that will always be used. Copyright
 * 2004 3SP <a href="http://localhost">3SP </a>.
 */
public class BrowserLauncher {
  ...
  private static final String JDirect_MacOSX = "/System/Library/Frameworks/Carbon.framework/Frameworks/HIToolbox.framework/HIToolbox"; //$NON-NLS-1$
  ...
  private static boolean loadClasses() {
    switch (jvm) {
      ...
      case MRJ_2_1 :
        try {
          mrjFileUtilsClass = Class.forName("com.apple.mrj.MRJFileUtils"); //$NON-NLS-1$
          mrjOSTypeClass = Class.forName("com.apple.mrj.MRJOSType"); //$NON-NLS-1$
          Field systemFolderField = mrjFileUtilsClass.getDeclaredField("kSystemFolderType"); //$NON-NLS-1$
          kSystemFolderType = systemFolderField.get(null);
          findFolder = mrjFileUtilsClass.getDeclaredMethod("findFolder", new Class[]{mrjOSTypeClass}); //$NON-NLS-1$
          getFileCreator = mrjFileUtilsClass.getDeclaredMethod("getFileCreator", new Class[]{File.class}); //$NON-NLS-1$
          getFileType = mrjFileUtilsClass.getDeclaredMethod("getFileType", new Class[]{File.class}); //$NON-NLS-1$
      ...
      case MRJ_COCOA :
        String className;
        if (jvm == MRJ_3_1) {
          className = "com.apple.mrj.MRJFileUtils"; //$NON-NLS-1$
        } else {
          className = "com.apple.eio.FileManager"; //$NON-NLS-1$
        }
        try {
          mrjFileUtilsClass = Class.forName(className);
          openURL = mrjFileUtilsClass.getDeclaredMethod("openURL", new Class[]{String.class}); //$NON-NLS-1$
        } catch (ClassNotFoundException cnfe) {
          errorMessage = cnfe.getMessage();
          return false;
        } catch (NoSuchMethodException nsme) {
  ...
  private static boolean loadMRJ20Classes() {
    try {
      ...
      makeOSType = osUtilsClass.getDeclaredMethod("makeOSType", new Class[]{String.class}); //$NON-NLS-1$
      putParameter = appleEventClass.getDeclaredMethod("putParameter", new Class[]{int.class, aeDescClass}); //$NON-NLS-1$
      sendNoReply = appleEventClass.getDeclaredMethod("sendNoReply", new Class[]{}); //$NON-NLS-1$

      Field keyDirectObjectField = aeClass.getDeclaredField("keyDirectObject"); //$NON-NLS-1$
      keyDirectObject = (Integer) keyDirectObjectField.get(null);
      Field autoGenerateReturnIDField = appleEventClass.getDeclaredField("kAutoGenerateReturnID"); //$NON-NLS-1$
      kAutoGenerateReturnID = (Integer) autoGenerateReturnIDField.get(null);
      Field anyTransactionIDField = appleEventClass.getDeclaredField("kAnyTransactionID"); //$NON-NLS-1$
  ...
  private static Object locateBrowser() {
    ...
    switch (jvm) {
      ...
      case MRJ_3_0 :
      case MRJ_3_1 :
        browser = ""; // Return something non-null //$NON-NLS-1$
        break;
      case WINDOWS_NT :
        browser = "cmd.exe"; //$NON-NLS-1$
        break;
      case WINDOWS_9x :
        browser = "command.com"; //$NON-NLS-1$
        break;
      case OTHER :
      default :
  ...
  public static void openURL(String url) throws IOException {
    if (!loadedWithoutErrors) {
      throw new IOException(MessageFormat.format(Messages.getString("BrowserLauncher.exceptionFindingBrowser"), new Object[] { errorMessage })); //$NON-NLS-1$
    }
    ...
    if (browser == null) {
      throw new IOException(MessageFormat.format(Messages.getString("BrowserLauncher.unableToLocateBrowser"), new Object[] { errorMessage } ) ); //$NON-NLS-1$
    }
    ...
    switch (jvm) {
      case MRJ_2_0 :
        ...
        try {
          ...
          throw new IOException(MessageFormat.format(Messages.getString("BrowserLauncher.invocationTargetException"), new Object[] { ite.getMessage() } ) ); //$NON-NLS-1$
        } catch (IllegalAccessException iae) {
          throw new IOException(MessageFormat.format(Messages.getString("BrowserLauncher.illegalAccess"), new Object[] { iae.getMessage() } )); //$NON-NLS-1$
        } catch (InstantiationException ie) {
          throw new IOException(MessageFormat.format(Messages.getString("BrowserLauncher.instantiation"), new Object[] { ie.getMessage() } )); //$NON-NLS-1$
        } finally {
      ...
      case MRJ_3_0 :
        ...
        if (result == 0) {
          ...
          if (result == 0) {
            ...
            throw new IOException(MessageFormat.format(Messages.getString("BrowserLauncher.cannotLaunchURL"), new Object[] { new Integer(result) } )); //$NON-NLS-1$
          }
        } else {
          throw new IOException(MessageFormat.format(Messages.getString("BrowserLauncher.cannotCreateNetConfg"), new Object[] { new Integer(result) } ) ); //$NON-NLS-1$
        }
        break;
      case MRJ_3_1 :
      case MRJ_COCOA :
        try {
          ...
          throw new IOException(MessageFormat.format(Messages.getString("BrowserLauncher.cannotOpenURL"), new Object[] { ite.getMessage() } ) ); //$NON-NLS-1$
        } catch (IllegalAccessException iae) {
          throw new IOException(MessageFormat.format(Messages.getString("BrowserLauncher.ilegalAccessOpeningURL"), new Object[] { iae.getMessage() } ) ); //$NON-NLS-1$
        }
        break;
      case WINDOWS_NT :
      case WINDOWS_9x :
        ...
        try {
          ...
          throw new IOException(Messages.getString("BrowserLauncher.interruptedLaunchingBrowser") + ie.getMessage()); //$NON-NLS-1$
        }
        break;
      case OTHER :
        ...
        try {
          ...
          throw new IOException(MessageFormat.format(Messages.getString("BrowserLauncher.interruptedLaunchingBrowser"), new Object[] { ie.getMessage() } ) ); //$NON-NLS-1$
        }
        break;
      default :
  ...
  private static String getEnvironmentBrowser() {
    String browser = null;
    try {
      String[] echoParams = {"/bin/sh", "-c", "echo ${BROWSER:-netscape}"}; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
