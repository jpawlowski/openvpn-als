...
2) Go to ovpnals-language directory (where build.xml is) and execute "ant extract".
   The target extract will extract all ApplicationResources.properties from other
   projects and copy them to [OVPNALS_LANGUAGE_PROJECT_DIR]/src/code.lang.
...
Example:
     ...
     create a new Directory "fr-FR" in [OVPNALS_LANGUAGE_PROJECT_DIR]/src/. This
     ...
     part of ovpnals, I have to load in Omega-T this file:

     [OVPNALS_LANGUAGE_PROJECT_DIR]/src/fr-FR/com/ovpnals/install/ApplicationResources.properties
     
     OmegaT will create a properties file in target directory. You must copy it
     (and possibly rename it) to [OVPNALS_LANGUAGE_PROJECT_DIR]/src/fr-FR/com/ovpnals/install/.
     ...
     to call an ant task in [OVPNALS_LANGUAGE_PROJECT_DIR]:
     ...
     the extension zip is created in [OVPNALS_LANGUAGE_PROJECT_DIR]. You can copy it to
     [OVPNALS_BIN]/conf/repository/archives or upload it via the WebUI. After that you can
