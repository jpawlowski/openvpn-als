...
import net.openvpn.als.core.CoreMessageResources;
...
import net.openvpn.als.extensions.ExtensionInstaller;
...
import net.openvpn.als.tasks.TaskHttpServletRequest;
import net.openvpn.als.tasks.TaskInputStream;
import net.openvpn.als.tasks.TaskProgressBar;
 ...
 * <p>
 * This class also manages the interaction with the <i>OpenVPN-ALS Extension Store</i>,
 * including update checking and installing from the store.
 */
public class ExtensionStore {

    public static final String HTTP_3SP_COM_APPSTORE = "http://www.openvpnalsproject.org/appstore/";
	...
	public void init(File basedir) throws IOException {
		...
		repositoryBacked = "true".equals(SystemProperties.get("openvpnals.extensions.repositoryBacked", "true"));
	...
	void checkExtensionsForInstallation() {
		...
		PropertyList forceInstalls = new PropertyList(SystemProperties.get("openvpnals.forceInstallers", ""));
		for (ExtensionBundle bundle : extensionBundlesList) {
			if (bundle.getInstaller().getOpCount() > 0 && bundle.getStatus().isStartedOrActivated()) {
				...
				if (force || ver.equals("") || !ver.equals(bundle.getVersion().toString())) {
				    if(force) {
                        log.info("Will run installer for " + bundle.getId() + " because it has been forced by the openvpnals.foreceInstallers property.");
				    }
				    else if (ver.equals("")) {
	...
	private static URL getDownloadURL(String id, String version) {
		try {
            String location = SystemProperties.get("openvpnals.downloadableApplicationStore.location", ExtensionStore.HTTP_3SP_COM_APPSTORE);
            location += Util.urlEncode(id) + "/" + Util.urlEncode(version) + "/" + Util.urlEncode(id) + ".zip";
			return new URL(location);
		} catch (MalformedURLException murle) {
			try {
				String path = SystemProperties.get("openvpnals.downloadableApplications.location");
				path = path.replaceAll("\\$\\{id\\}", id);
                path = path.replaceAll("\\$\\{version\\}", version);
				return new File(path).toURL();
			} catch (MalformedURLException e) {
				log.error("Invalid downloadable extension location specified in system property openvpnals.downloadableApplicationStore.location, '" + SystemProperties.get("openvpnals.downloadableApplicationStore.location")
	...
	public static URL getStoreDownloadURL(String appStoreLocation, VersionInfo.Version version) {
		try {
			String location = SystemProperties.get("openvpnals.downloadableApplicationStore.location", appStoreLocation);
            location += "core/" + Util.urlEncode(version.toString()) + "/store.xml";
			return new URL(location);
		} catch (MalformedURLException murle) {
			try {
				return new File(SystemProperties.get("openvpnals.downloadableApplicationStore.location")).toURL();
			} catch (MalformedURLException e) {
				log.error("Invalid downloadable extension store location specified in system property openvpnals.downloadableApplicationStore.location, '" + SystemProperties.get("openvpnals.downloadableApplicationStore.location")
					+ "'. Must be either a URL or the file path of the store descriptor file.");
			}
		}
		return null;
	...
	private void loadAll() throws Exception {
		...
		String descriptors = SystemProperties.get("openvpnals.additionalDescriptors", "");
    ...
    public static VersionInfo.Version getWorkingVersion() {
        VersionInfo.Version version = new VersionInfo.Version(SystemProperties.get("openvpnals.forceVersion", ContextHolder.getContext().getVersion().toString()));
        return version;
    }
	...
	/**
	 * Install an extension directly from the OpenVPN-ALS Extension Store given its id
	...
	public ExtensionBundle installExtensionFromStore(final String id, InputStream in, HttpServletRequest request, long contentLength)
		...
		try {
			...
			if (bundle.getRequiredHostVersion() != null && bundle.getRequiredHostVersion().compareTo(context.getVersion()) > 0) {
				throw new ExtensionException(ExtensionException.INSUFFICIENT_OpenVPNALS_HOST_VERSION,
								bundle.getId(),
	 ...
	 * OpenVPN-ALS Application Store or manually.
	 * 
...
//		}
	...
	private void addAdditionalWebResource() {
		//
		String additionalWebResources = SystemProperties.get("openvpnals.additionalWebResourceDirectories", "");
	...
	private void addAdditionalClasspath() {
		...
		// Add any additional class path elements
		StringTokenizer t = new StringTokenizer(SystemProperties.get("openvpnals.additionalClasspath", ""), ",");
		while (t.hasMoreTokens()) {
			try {
	...
	private void loadDevExtensions() {
		...
		String extensionList = SystemProperties.get("openvpnals.devExtensions", "");
		...
		while (t.hasMoreTokens()) {
			...
			if (ext.equalsIgnoreCase("all")) {
				...
				File[] dirs = f.listFiles(new FileFilter() {
					public boolean accept(File pathname) {
						File f = new File(pathname, "extensions");
						return f.exists() && f.isDirectory() && pathname.getName().indexOf("openvpnals-enterprise-") != -1;
					}
				});
				for (int i = 0; dirs != null && i < dirs.length; i++) {
					devExtensions.add(dirs[i].getName());
				...
				File[] dirs = f.listFiles(new FileFilter() {
					public boolean accept(File pathname) {
						File f = new File(pathname, "extensions");
						return f.exists() && f.isDirectory() && pathname.getName().indexOf("openvpnals-community-") != -1;
					}
				});
				for (int i = 0; dirs != null && i < dirs.length; i++) {
					devExtensions.add(dirs[i].getName());
