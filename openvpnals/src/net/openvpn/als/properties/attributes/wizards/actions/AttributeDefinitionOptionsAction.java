...
import net.openvpn.als.properties.attributes.wizards.forms.AttributeDefinitionOptionsForm;
...
public class AttributeDefinitionOptionsAction extends AbstractWizardAction {
	...
	public ActionForward typeChanged(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(adof.getType() == PropertyDefinition.TYPE_COLOR ||
			...
			getWizardSequence(request).putAttribute(AttributeDefinitionOptionsAction.ATTR_VALIDATION_STRING, "net.openvpn.als.input.validators.BooleanValidator");			
		}
		else if(adof.getType() == PropertyDefinition.TYPE_INTEGER) {
			getWizardSequence(request).putAttribute(AttributeDefinitionOptionsAction.ATTR_VALIDATION_STRING, "net.openvpn.als.input.validators.IntegerValidator(minValue=0,maxValue=9999999)");
		}
		else if(adof.getType() == PropertyDefinition.TYPE_STRING) {
			getWizardSequence(request).putAttribute(AttributeDefinitionOptionsAction.ATTR_VALIDATION_STRING, "net.openvpn.als.input.validators.StringValidator(minLength=0,maxLength=30,trim=true,regExp=,pattern=)");
		}
		else if(adof.getType() == PropertyDefinition.TYPE_TEXT_AREA) {
			getWizardSequence(request).putAttribute(AttributeDefinitionOptionsAction.ATTR_VALIDATION_STRING, "net.openvpn.als.input.validators.StringValidator(minLength=0,maxLength=255,trim=true,regExp=,pattern=)");
		}
