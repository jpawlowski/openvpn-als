...
import net.openvpn.als.core.PageInterceptException;
import net.openvpn.als.core.PageInterceptListener;
...
import net.openvpn.als.properties.ProfilesListDataSource;
...
import net.openvpn.als.security.actions.PromptForPrivateKeyPassphraseDispatchAction;
import net.openvpn.als.security.actions.UpdatePrivateKeyPassphraseDispatchAction;
import net.openvpn.als.setup.SystemInformationProvider;
 ...
 * {@link net.openvpn.als.security.LogonController} and maintains and validates
 * all logons to OpenVPN-ALS whether the be through the web based user
...
public class DefaultLogonController implements LogonController {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see net.openvpn.als.security.LogonController#init()
	 */
	public void init() {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see net.openvpn.als.security.LogonController#isAdministrator(net.openvpn.als.policyframework.Principal)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see net.openvpn.als.security.LogonController#addSessionTimeoutBlock(javax.servlet.http.HttpSession,
	 *      java.lang.String)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see net.openvpn.als.security.LogonController#removeSessionTimeoutBlock(javax.servlet.http.HttpSession,
	 *      int)
	 */
	...
	public void initialiseSession(HttpSession session, User user) throws UserDatabaseException {
		...
		String logonInfo = MessageResources.getMessageResources("net.openvpn.als.navigation.ApplicationResources")
	...
	public AccountLock checkForAccountLock(String username, String realmName) throws SecurityErrorException,
		...
		AccountLock lock = "true".equals(SystemProperties.get("openvpnals.recoveryMode", "false")) ? null
	...
	// // Get the current lock (if any)
	// AccountLock lock =
	// "true".equals(SystemProperties.get("openvpnals.recoveryMode", "false")) ?
	// null
	// : (AccountLock) lockedUsers.get(username);
	// // If the user is currently locked, check if the lock has expired yeet
	...
	private void refreshLogonTicket(HttpServletRequest request, HttpServletResponse response, String logonTicket)
		 ...
		 */
		try {
			String sessionIdentifier = SystemProperties.get("openvpnals.cookie", "JSESSIONID");
	 ...
	 * (non-Javadoc)
	 * 
	 * @see net.openvpn.als.security.LogonController#applyMenuItemChanges(javax.servlet.http.HttpServletRequest)
	 */
    	                ...
    	                for(Iterator i = sessions.values().iterator(); i.hasNext(); ) {
    	                    ...
    	                    if(sesh.getHttpSession() != null) {
    	                        try {
    	                            ...
    	                            log.error("------------------------------------------------------");
    	                            log.error("An OpenVPN-ALS session that is attached to an");
