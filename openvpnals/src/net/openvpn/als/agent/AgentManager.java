...
public class AgentManager {
    ...
    AgentManager() {
        // The default type of agent is the OpenVPN-ALS Agent
        types.put(DefaultAgentCallback.DEFAULT_AGENT_TYPE, new DefaultAgentCallback());
        if (SystemProperties.get("openvpnals.agent.debug", "").equals("enabled")) {
            types.put(DefaultAgentCallback.DEFAULT_AGENT_TYPE, new DummyAgentCallback());
        } else if (SystemProperties.get("openvpnals.agent.debug", "").equals("interactive")) {
            types.put(DefaultAgentCallback.DEFAULT_AGENT_TYPE, new InteractiveAgentCallback());
        } else if (SystemProperties.get("openvpnals.agent.debug", "").equals("attach")) {
