 ...
 * <i>hostname[:port]</i> or <i>ipAddress[:port]</i>.  
 */
public class HostnameOrIPAddressWithPortValidator extends StringValidator {
	
	private static final int MAX_HOST_OR_IP_CHARS = 255;
    /**
	 * Regular expression for a string in the format <i>hostname[:port]</i> or
	 * <i>ipAddress[:port]</i>.
	 */
	final static String HOST_PATTERN = "^[a-zA-Z0-9\\.\\-\\_]*((\\:[0-9]*)?)$";

	/**
	 * Constructor.
	 */
	public HostnameOrIPAddressWithPortValidator() {
		super(1, MAX_HOST_OR_IP_CHARS, HOST_PATTERN, null, true); 
		this.regExpErrCode = ErrorConstants.ERR_STRING_ISNT_HOSTNAME_OR_IPADDRESS_WITH_PORT;
	}
