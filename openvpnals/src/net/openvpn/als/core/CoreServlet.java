...
import net.openvpn.als.agent.AgentRequestHandler;
...
import net.openvpn.als.extensions.ApplicationParameters;
...
import net.openvpn.als.jdbc.hsqldb.EmbeddedHSQLDBServer;
...
import net.openvpn.als.keystore.wizards.types.RootServerCertificateImportType;
import net.openvpn.als.keystore.wizards.types.ServerAuthenticationKeyImportType;
import net.openvpn.als.keystore.wizards.types.TrustedServerCertificateImportType;
import net.openvpn.als.keystore.wizards.types._3SPPurchaseImportType;
import net.openvpn.als.language.Language;
import net.openvpn.als.language.LanguagePackDefinition;
import net.openvpn.als.language.LanguagePackManager;
import net.openvpn.als.navigation.NavigationBar;
import net.openvpn.als.navigation.NavigationManager;
import net.openvpn.als.notification.Notifier;
import net.openvpn.als.notification.agent.AgentMessageSink;
import net.openvpn.als.notification.smtp.SMTPMessageSink;
...
import net.openvpn.als.requesthandler.test.TestRequestHandler;
import net.openvpn.als.rss.FeedManager;
import net.openvpn.als.security.AuthenticationModuleManager;
import net.openvpn.als.security.Constants;
import net.openvpn.als.security.EmbeddedClientAuthenticationModule;
import net.openvpn.als.security.HTTPAuthenticationModule;
import net.openvpn.als.security.LogonControllerFactory;
import net.openvpn.als.security.PasswordAuthenticationModule;
...
import net.openvpn.als.services.CoreEventService;
import net.openvpn.als.services.CoreEventServiceImpl;
import net.openvpn.als.services.MessageResourceService;
...
public class CoreServlet extends ActionServlet implements ContextListener, MessageResourceService {
     ...
     * (non-Javadoc)
     * 
     * @see net.openvpn.als.core.CoreEventManager#addCoreListener(net.openvpn.als.core.CoreListener)
     */
     ...
     * (non-Javadoc)
     * 
     * @see net.openvpn.als.core.CoreEventManager#removeCoreListener(net.openvpn.als.core.CoreListener)
     */
     ...
     * (non-Javadoc)
     * 
     * @see net.openvpn.als.core.CoreEventManager#fireCoreEvent(net.openvpn.als.core.CoreEvent)
     */
    ...
    protected void initOther() throws ServletException {
        ...
        applicationStoreResources = (CoreMessageResources) messageResourcesFactory
                        .createResources("net.openvpn.als.applications.store.ApplicationResources");
    ...
    protected void activateCore() throws ServletException {
        ...
        if ("true".equalsIgnoreCase(SystemProperties.get("openvpnals.disableExtensions"))) {
        ...
        if (!ContextHolder.getContext().isSetupMode()) {
            if (SystemProperties.get("openvpnals.testing", "false").equals("true")) {
        ...
        if (!Property.getPropertyBoolean(new SystemConfigKey("ui.rssFeeds")) 
        		|| SystemProperties.get("openvpnals.disable.rssFeeds", "false").equals("true")) {
            if (log.isInfoEnabled())
    ...
    public void init() throws ServletException {
        ...
        try {
            ...
            if (SystemProperties.get("openvpnals.disableNewSSLEngine", "false").equals("true"))
            ...
            dbServer = new EmbeddedHSQLDBServer("true".equalsIgnoreCase(SystemProperties.get("openvpnals.hsqldb.tcpipServer",
                "false")));
    ...
    protected ModuleConfig initModuleConfig(String prefix, String paths) throws ServletException {
        ...
        moduleConfig.getControllerConfig().setProcessorClass("net.openvpn.als.core.CoreRequestProcessor");
        return moduleConfig;
    }
    ...
    protected void initCore() throws ServletException {
         ...
         */
        if (!("false".equals(SystemProperties.get("openvpnals.useMaverickSSL", "true")))) {
            try {
                if (log.isInfoEnabled())
    ...
    void checkDevEnvironment() throws ServletException {
        devConfig = "true".equalsIgnoreCase(SystemProperties.get("openvpnals.useDevConfig", "false"));
