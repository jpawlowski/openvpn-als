...
public class CoreRequestProcessor extends TilesRequestProcessor {
	...
	private void setDefaultLocale(HttpServletRequest request, HttpSession session) {
		...
		try {
            if (Util.isNullOrTrimmedBlank(SystemProperties.get("openvpnals.defaultLanguage", ""))) {
    		    Cookie[] c = request.getCookies();
    		    if (c != null) {
    		        for (int i = 0; i < c.length; i++) {
    		            if (c[i].getName().equals(SystemProperties.get("openvpnals.cookie", "SSLX_SSESHID") + "_LANG")) {
                ...
                defaultLocale = SystemProperties.get("openvpnals.defaultLanguage", "");
            }
		    Locale locale;
    ...
    private static boolean isRunningUnitTests() {
        String isRunningUnitTests = SystemProperties.get("openvpnals.testing", "false");
