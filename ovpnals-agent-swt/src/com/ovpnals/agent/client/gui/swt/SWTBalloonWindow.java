 ...
 * @since Jul 2, 2004
 * @version $Id: SWTBalloonWindow.java,v 1.1 2007/02/08 11:44:35 brett Exp $
 */

public class SWTBalloonWindow {
	...
	private int autoLocation = SWT.NONE;
	...
	public SWTBalloonWindow(Shell parent, int style) {
		this(null, parent, style);
	}

	public SWTBalloonWindow(Display display, int style) {
		this(display, null, style);
	}

	private SWTBalloonWindow(Display display, Shell parent, final int style) {
		this.style = style;
	...
	public void prepareForOpen() {
		...
		final Point shellSize = (anchor == SWT.NONE) ? new Point(contentsSize.x + marginLeft + marginRight,
						contentsSize.y + marginTop + marginBottom) : new Point(contentsSize.x + marginLeft + marginRight,
		...
		if (autoLocation != SWT.NONE) {
			Point shellLoc = new Point(locX == Integer.MIN_VALUE ? 0 : locX, locY == Integer.MIN_VALUE ? 0 : locY);
			Rectangle screenBounds = shell.getDisplay().getPrimaryMonitor().getBounds();
			if ((autoLocation & SWT.RIGHT) != 0) {
				shellLoc.x = screenBounds.width - shellSize.x - shellLoc.x;				
			}
			if ((autoLocation & SWT.BOTTOM) != 0) {
				shellLoc.y = screenBounds.height - shellSize.y  - shellLoc.y;
			}
