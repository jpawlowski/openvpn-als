...
<!-- $Header: /cvsroot/build-tools/unit-testing/pmd/wz-pmd-report.xslt,v 1.1.2.2 2007/07/14 23:30:27 karl Exp $ -->
...
<html>
<head>
    <title>OpenVPNALS PMD Report</title>
    ...
    <style type="text/css">
        ...
        table.summary tr th { font-weight: bold; text-align:left; background:#a6caf0;margin:10em }
...
<body>
    <H1><div class="top"><div class="left">OpenVPNALS - PMD Report</div><div class="right"><xsl:call-template name="timestamp"/></div></div></H1>
    <hr/>
    <h2>Summary</h2>
    <table border="0" class="summary">
      ...
      <tr>
        <td style="padding:3px" width="10%"><xsl:value-of select="count(//file)"/></td>
        <td style="padding:3px" width="10%"><xsl:value-of select="count(//violation)"/></td>
        <td style="padding:3px" width="10%"><div class="p1"><xsl:value-of select="count(//violation[@priority = 1])"/></div></td>
        <td style="padding:3px" width="10%"><div class="p2"><xsl:value-of select="count(//violation[@priority = 2])"/></div></td>
        <td style="padding:3px" width="10%"><div class="p3"><xsl:value-of select="count(//violation[@priority = 3])"/></div></td>
        <td style="padding:3px" width="10%"><div class="p4"><xsl:value-of select="count(//violation[@priority = 4])"/></div></td>
        <td style="padding:3px" width="10%"><div class="p5"><xsl:value-of select="count(//violation[@priority = 5])"/></div></td>
      </tr>
    </table>
    <hr/>
	...
	<xsl:for-each select="file">
	    <xsl:for-each select="violation">
		    <tr>
				<td style="padding:3px" align="middle"><div><xsl:attribute name="class"><xsl:call-template name="priorityDiv"/></xsl:attribute><xsl:value-of disable-output-escaping="yes" select="@priority"/></div></td>
				<td style="padding:3px" align="left"><xsl:value-of disable-output-escaping="yes" select="substring-before(translate(../@name,'/','.'),'.java')"/></td>
				<td style="padding:3px" align="middle"><xsl:value-of disable-output-escaping="yes" select="@beginline"/></td>
	...
	<br/>
    <p><i>Copyright &#169; 2003-2007 3SP LTD. All Rights Reserved.</i></p>
</body>
</html>
</xsl:template>
